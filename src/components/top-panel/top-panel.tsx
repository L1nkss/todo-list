...
const createFilterButtons = (active: TFilters, onClickCallback: (type: TFilters) => void): Array<React.ReactNode> => {
    return FILTERS_BUTTONS.map((button) => {
        const className = active === button.label ?  'button--active' : 'button--grey';

        return <button key={button.label} onClick={() => onClickCallback(button.label)} type="button" className={`button ${className}`}>{button.name}</button>
    })
};
